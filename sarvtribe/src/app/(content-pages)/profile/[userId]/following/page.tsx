// src/app/profile/[userId]/following/page.tsx
'use client';
...
export default function FollowingPage() {
  ...
  const { data: following, error, isLoading } = useSWR<Partial<User>[]>(
  ...
  return (
    <div className="container mx-auto max-w-2xl mt-8 px-4">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden">
        {isLoading && (
          <p className="text-gray-500 dark:text-gray-400 p-6 text-center">Loading following...</p>
        )}
        {error && <p className="text-red-500 p-6 text-center">Failed to load list.</p>}

        {following?.length ? (
          <div className="divide-y divide-gray-200 dark:divide-gray-800">
            {following.map(user => (
              <UserListItem key={user.id} user={user} listType="following" mutate={() => {}} />
            ))}
          </div>
        ) : (
          !isLoading && !error && (
