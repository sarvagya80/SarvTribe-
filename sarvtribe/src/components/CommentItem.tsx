      ...
      <div className="flex-1">
        <div className="bg-gray-100 dark:bg-gray-800 p-3 rounded-lg">
          <div className="flex items-center space-x-2">
            ...
            <p className="text-xs text-gray-400">{formatDistanceToNowStrict(new Date(comment.createdAt))} ago</p>
          </div>
          <p className="mt-1 text-sm">{comment.body}</p>
        </div>
        <div className="flex items-center space-x-3 text-xs mt-1 text-gray-400">
          <button onClick={handleLike} className="font-semibold flex items-center space-x-1">
        ...
        {isReplying && (
          <form onSubmit={handleReplySubmit} className="mt-2 flex items-center gap-2">
            <input 
              ...
              autoFocus
            />
            <button type="submit" className="px-3 py-1 text-xs rounded-md bg-indigo-500 text-white hover:bg-indigo-600">Post</button>
          </form>
        )}
